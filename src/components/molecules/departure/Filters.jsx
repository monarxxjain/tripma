...
const Filters = () => {
    ...
    // Use an array to store refs for each dropdown
    const dropdownRefs = filters.map(() => useRef());
            ...
            {filters.map((filter, index) => {
                return (
                    <div key={index} className='py-2 px-3 shrink-0 flex justify-between rounded border border-[#CBD4E6]'>
                        <select ref={dropdownRefs[index]} className='z-10 bg-transparent'>
                        ...
                        <Image src={Drop} onClick={() => dropdownRefs[index].current.click()} className='cursor-pointer ms-[-15px]' />
                    </div>
                )
            })}
        </div>
